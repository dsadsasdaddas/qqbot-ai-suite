...
CC_MODE_STATE_PATH = Path("data/config/claude_glm_mobile_state.json")
...
def load_mode_state() -> Dict[str, Any]:
    try:
        if CC_MODE_STATE_PATH.exists():
            obj = json.loads(CC_MODE_STATE_PATH.read_text(encoding="utf-8"))
            if isinstance(obj, dict):
                obj.setdefault("sessions", {})
                return obj
    except Exception as exc:
        logger.warning(f"claude_glm_mobile mode 状态读取失败: {exc}")
    return {"sessions": {}}


def save_mode_state(state: Dict[str, Any]) -> None:
    CC_MODE_STATE_PATH.parent.mkdir(parents=True, exist_ok=True)
    CC_MODE_STATE_PATH.write_text(json.dumps(state, ensure_ascii=False, indent=2), encoding="utf-8")


def session_state(state: Dict[str, Any], sid: str) -> Dict[str, Any]:
    sessions = state.setdefault("sessions", {})
    item = sessions.setdefault(str(sid), {})
    item.setdefault("enabled", False)
    item.setdefault("active_task", "default")
    item.setdefault("tasks", {"default": {"name": "default"}})
    return item


def is_allowed_sender(sender_id: Any, cfg: Dict[str, Any]) -> bool:
    allowed = [str(x) for x in cfg.get("allowed_user_ids", []) if str(x).strip()]
    return (not allowed) or str(sender_id) in allowed


def cc_session_enabled(event: AstrMessageEvent) -> bool:
    try:
        cfg = load_cfg()
        if not is_allowed_sender(event.get_sender_id(), cfg):
            return False
        sid = str(getattr(event, "unified_msg_origin", "") or "")
        if not sid:
            return False
        state = load_mode_state()
        item = (state.get("sessions") or {}).get(sid) or {}
...
def first_word(text: str) -> str:
    return re.split(r"\s+", (text or "").strip(), maxsplit=1)[0].strip().lstrip("/!！／").lower()


class CCModeFilter(CustomFilter):
    """CC 模式：会话开启后，普通打字直接提交 Claude Code job。"""

    def filter(self, event: AstrMessageEvent, cfg) -> bool:  # noqa: ANN001
        if not cc_session_enabled(event):
            return False
        text = (event.get_message_str() or "").strip()
        if not text:
            return False
        # 明确保留这些给 AstrBot 控制层；其它斜杠在 CC 模式下尽量交给 Claude。
        fw = first_word(text)
        if fw in {"cc", "claude", "claudecode", "克劳德", "ccmode", "cctask"}:
            return False
        return True
...
class Main(star.Star):
    ...
    async def cc_command(self, event: AstrMessageEvent):
        ...
        try:
            ...
            if action in {"state", "memory", "上下文", "状态文件"}:
                ...
                return
            if action in {"mode", "模式"}:
                yield event.plain_result(self._mode_cmd(event, rest))
                return
            if action in {"task", "任务槽", "slot"}:
                yield event.plain_result(self._task_cmd(event, rest))
                return
            if action in {"raw", "slash", "斜杠"}:
                task = rest.strip()
                yield event.plain_result(await self._submit_job(event, cfg, self._decorate_mode_task(event, task), continue_session=True))
                return
    ...
    @filter.command("ccmode", alias={"cc模式"})
    async def ccmode_command(self, event: AstrMessageEvent):
        event.call_llm = False
        cfg = load_cfg()
        if not self._is_allowed(event, cfg):
            yield event.plain_result("Claude(GLM) 长程 agent 当前只对白名单用户开放。")
            return
        arg = re.sub(r"^[/!！\s]*(?:ccmode|cc模式)\b", "", event.get_message_str() or "", flags=re.I).strip()
        yield event.plain_result(self._mode_cmd(event, arg or "status"))

    @filter.command("cctask", alias={"cc任务"})
    async def cctask_command(self, event: AstrMessageEvent):
        event.call_llm = False
        cfg = load_cfg()
        if not self._is_allowed(event, cfg):
            yield event.plain_result("Claude(GLM) 长程 agent 当前只对白名单用户开放。")
            return
        arg = re.sub(r"^[/!！\s]*(?:cctask|cc任务)\b", "", event.get_message_str() or "", flags=re.I).strip()
        yield event.plain_result(self._task_cmd(event, arg or "current"))

    @filter.custom_filter(CCModeFilter)
    async def cc_mode_direct_message(self, event: AstrMessageEvent):
        event.call_llm = True
        cfg = load_cfg()
        if not self._is_allowed(event, cfg):
            return
        text = (event.get_message_str() or "").strip()
        task = self._normalize_cc_mode_text(text)
        if not task:
            return
        yield event.plain_result(await self._submit_job(event, cfg, self._decorate_mode_task(event, task), continue_session=True))
    ...
    def _help(self) -> str:
        return (
            ...
            "/cc stop <job_id> 取消任务\n"
            "/cc continue 继续上次任务\n"
            "/cc mode on|off|status 开关当前会话 CC 直聊模式\n"
            "/cc task new|use|list|current <name> 切换任务槽\n"
            "CC 模式下：普通打字直接进 Claude Code；/compact 等斜杠尽量直送 Claude。若被 AstrBot 抢走，用 //compact 或 /cc /compact。"
        )

    def _mode_cmd(self, event: AstrMessageEvent, rest: str) -> str:
        sid = str(getattr(event, "unified_msg_origin", "") or "")
        state = load_mode_state()
        item = session_state(state, sid)
        action, _ = self._split(rest or "status")
        if action in {"on", "开", "开启", "enable", "enabled"}:
            item["enabled"] = True
            save_mode_state(state)
            return self._mode_status_text(event, item, prefix="CC 模式已开启。现在普通打字会直接提交 Claude Code job。")
        if action in {"off", "关", "关闭", "disable", "disabled"}:
            item["enabled"] = False
            save_mode_state(state)
            return self._mode_status_text(event, item, prefix="CC 模式已关闭。")
        return self._mode_status_text(event, item)

    def _mode_status_text(self, event: AstrMessageEvent, item: Dict[str, Any], prefix: str = "") -> str:
        lines = []
        if prefix:
            lines.append(prefix)
        lines += [
            f"enabled: {bool(item.get('enabled'))}",
            f"active_task: {item.get('active_task') or 'default'}",
            "用法：",
            "- /ccmode on 或 /cc mode on：开启当前会话直聊",
            "- /ccmode off：关闭",
            "- /cctask use <name>：切换任务槽",
            "- CC模式下普通打字=提交 /cc job",
            "- Claude 斜杠：优先直接发 /compact；若被 AstrBot 抢走，用 //compact 或 /cc /compact",
        ]
        return "\n".join(lines)

    def _task_cmd(self, event: AstrMessageEvent, rest: str) -> str:
        sid = str(getattr(event, "unified_msg_origin", "") or "")
        state = load_mode_state()
        item = session_state(state, sid)
        action, name = self._split(rest or "current")
        name = (name or "").strip()
        tasks = item.setdefault("tasks", {"default": {"name": "default"}})
        if action in {"new", "add", "创建", "新增"}:
            if not name:
                return "需要任务槽名称：/cctask new 知识产权系统"
            tasks[name] = {"name": name}
            item["active_task"] = name
            save_mode_state(state)
            return f"已创建并切换任务槽：{name}"
        if action in {"use", "switch", "切换", "选", "选择"}:
            if not name:
                return "需要任务槽名称：/cctask use 知识产权系统"
            tasks.setdefault(name, {"name": name})
            item["active_task"] = name
            save_mode_state(state)
            return f"已切换任务槽：{name}"
        if action in {"list", "ls", "列表"}:
            active = item.get("active_task") or "default"
            rows = [f"{'* ' if k == active else '- '}{k}" for k in sorted(tasks.keys())]
            return "CC 任务槽：\n" + "\n".join(rows)
        if action in {"current", "status", "当前", "状态"}:
            return f"当前任务槽：{item.get('active_task') or 'default'}"
        return "用法：/cctask new|use|list|current <name>"

    def _normalize_cc_mode_text(self, text: str) -> str:
        t = (text or "").strip()
        if t.startswith("//"):
            return "/" + t[2:].lstrip()
        if t.startswith("／"):
    ...
    def _decorate_mode_task(self, event: AstrMessageEvent, task: str) -> str:
        state = load_mode_state()
        sid = str(getattr(event, "unified_msg_origin", "") or "")
        item = session_state(state, sid)
        active = str(item.get("active_task") or "default")
